...
class ExampleVC: UINavigationController {
    ...
    public override func viewDidLoad() {
        ...
        tip.text = ExampleVC.textArray[ExampleVC.index]
        ExampleVC.index = (ExampleVC.index < ExampleVC.textArray.count - 1) ? ExampleVC.index + 1 : 0
        tip.numberOfLines = -1
        tip.textAlignment = .center
        
        icon.addTarget(self, action: #selector(ExampleVC.homePageAction), for: .touchUpInside)
        ...
        button.addTarget(self, action: #selector(ExampleVC.backAction), for: .touchUpInside)
